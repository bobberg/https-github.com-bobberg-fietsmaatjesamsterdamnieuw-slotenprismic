...
/**
 * Default allowed postal code prefixes for Amsterdam Nieuw-Sloten area.
 */
const DEFAULT_POSTAL_CODES = ['1060', '1065', '1066', '1068', '1069'];

/**
 * @typedef {import("@prismicio/client").Content.ContactFormValidatedSlice} ContactFormValidatedSlice
 * @typedef {import("@prismicio/react").SliceComponentProps<ContactFormValidatedSlice>} ContactFormValidatedProps
...
const ContactFormValidated = ({ slice }) => {
  ...
  // Parse allowed postal codes from Prismic field or use defaults
  const allowedCodes = slice.primary.allowed_postal_codes
    ? slice.primary.allowed_postal_codes.split(',').map((c) => c.trim()).filter(Boolean)
